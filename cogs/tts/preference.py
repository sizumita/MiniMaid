...
class UserPreferenceMixin(TTSPreferenceBase):
    ...
    async def preference(self, ctx: Context) -> None:
        async with self.bot.db.Session() as session:
            result = await session.execute(select_user_setting(ctx.author.id))
    ...
    async def tts_speed(self, ctx: Context, value: float) -> None:
    ...
    async def tts_volume(self, ctx: Context, value: float) -> None:
    ...
    async def tts_tone(self, ctx: Context, value: float) -> None:
    ...
    async def tts_intone(self, ctx: Context, value: float) -> None:
    ...
    async def tts_reset(self, ctx: Context) -> None:
...
class GuildPreferenceMixin(TTSPreferenceBase):
    ...
    @guild_only()
    async def guild_preference(self, ctx: Context) -> None:
        async with self.bot.db.Session() as session:
            result = await session.execute(select_guild_setting(ctx.guild.id))
            pref = result.scalars().first()
            if pref is None:
...
class VoiceDictionaryMixin(TTSPreferenceBase):
    ...
    @guild_only()
    async def voice_dictionary(self, ctx: Context) -> None:
        async with self.bot.db.Session() as session:
    ...
    async def remove_voice_dictionary(self, ctx: Context, before: str) -> None:
        async with self.bot.db.Session() as session:
            result = await session.execute(select_voice_dictionary(ctx.guild.id, before))
            dic = result.scalars().first()
            if dic is None:
